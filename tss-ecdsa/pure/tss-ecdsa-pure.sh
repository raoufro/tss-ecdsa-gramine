...
find ./gramine-config -type f ! -iname "manifest.template" -exec rm -f {} \;
...
for (( num_player=3; num_player<=$TOTAL_PLAYERS; num_player++ )); do
    ...
    for (( i=1; i<=$num_player; i++)); do
        ...
        if [ $i -eq ${num_player} ]; then
            sleep 5
            docker-compose run tss-player${i} "/root/scripts/tss-player-keygen-timed.sh \
                ${TSS_COORDINATOR_IPV4} ${TSS_COORDINATOR_PORT} ${num_player} ${threshold} ${i}"
        else
            sleep 5 # To Keep the order of DKG
    ...
    for (( i=1; i<=${threshold}; i++)); do
        ...
        if [ $i -eq ${threshold} ]; then
            sleep 5
            docker-compose run tss-player${i} "/root/scripts/tss-player-sign-timed.sh \
                ${TSS_COORDINATOR_IPV4} ${TSS_COORDINATOR_PORT} ${num_player} ${threshold} ${i} ${list_of_players}"
        else
            sleep 5 #To keep the order of siging
